...
public record Path(List<Element> elements) {
    ...
    public Path(final List<Element> elements) {
        ...
        if (!elements.isEmpty()) {
            Invariant.require(elements.get(0) instanceof Node, "first element of a non‐empty path must be a Node");
            Invariant.require(elements.get(elements.size() - 1) instanceof Node,
